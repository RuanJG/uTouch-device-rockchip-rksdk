...
BOARD_CONFIG=device/rockchip/rksdk/BoardConfig.mk
...
#  add tchip kernel version into root
if [ -f kernel/.fw_version ] ; then
    [ -d $OUT/root/ ] && cp kernel/.fw_version $OUT/root/tchip_kv
    [ -d $OUT/recovery/root/ ] && cp kernel/.fw_version $OUT/recovery/root/tchip_kv
fi
...
if [ -f "out/pcba_tmp/rockdev/update.img" ];then
    echo "create pcba.img..."
    cp out/pcba_tmp/rockdev/update.img rockdev/Image/pcba.img
fi

chmod a+r -R $IMAGE_PATH/

# Copy images to rk29utils dir
echo "Copy images to rk30 flash-burnning-utils directory ..."
cp -f rockdev/Image/* $HOME/vmware/vmShared
