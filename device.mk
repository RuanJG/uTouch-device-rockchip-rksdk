...
ifneq ($(strip $(BOARD_CONNECTIVITY_VENDOR)), nmi)
...
ifeq ($(strip $(BOARD_CONNECTIVITY_VENDOR)), nmi)
ifeq ($(strip $(NMC1000_WIFI_SUPPORT_P2P)),true)
include device/rockchip/common/features/rk-core.mk
else
include device/rockchip/common/features/rk-core-no-direct.mk
endif
else
include device/rockchip/common/features/rk-core.mk
endif
...
PRODUCT_USB_CONFIG ?= mass_storage,adb
	PRODUCT_PROPERTY_OVERRIDES += \
		ro.factory.hasUMS=true \
		persist.sys.usb.config=$(strip $(PRODUCT_USB_CONFIG)) 
...
#TchipTool add by zxx
PRODUCT_PACKAGES += \
    Tchip_SystemAttrTool \
    Tchip_ToolHelper \
    Tchip_Logcat    
...
# TchipTool Help.TXT add by zxx
PRODUCT_COPY_FILES += device/rockchip/rksdk/TchipHelp.txt:system/usr/data/TchipHelp.txt

# Tchip logsave add by zxx 
PRODUCT_COPY_FILES += device/rockchip/rksdk/cat_log.sh:system/bin/cat_log.sh
...
ifeq ($(strip $(BOARD_CONNECTIVITY_VENDOR)), nmi)
include hardware/nmi/nmc1xxx/wlan/config/config-nmi.mk
endif 

#########################################################
#		launcher ui
#########################################################
PRODUCT_COPY_FILES += $(LOCAL_PATH)/tchiplauncher/uiconfig:system/bin/uiconfig
